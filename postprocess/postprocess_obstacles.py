...
def plot_ensemble_top_view(ref_nocontrol, ref_control, ensemble_nc, ensemble_c):
    ...
    fig.suptitle("Obstacle Avoidance", fontsize=10, fontweight='bold', y=0.97)

    gs = gridspec.GridSpec(2, 1, height_ratios=[1, 1], hspace=0.05)

    color_rob, color_nom, color_obs, gate_color = '#0072B2', '#D55E00', '#999999', '#009E73'
    ...
    for i in range(90):
        ...
        ax1.plot(ensemble_nc[i, 10, ], ensemble_nc[i, 11, ],
                 color=color_nom, alpha=0.15, linewidth=0.5, zorder=3)
        lbl_c = 'Individual Runs (Robust)' if i == 0 else ""
        ax1.plot(ensemble_c[i, 10, :], ensemble_c[i, 11, :],
                 color=color_rob, alpha=0.15, linewidth=0.5, zorder=3)
    ax1.vlines(gate_x, gate_y - gate_radius, gate_y + gate_radius,

              colors=gate_color, linewidth=3, label='Target Gate', zorder=5)
    ax1.hlines([gate_y - gate_radius, gate_y + gate_radius], gate_x - 0.5, gate_x + 0.5,
              colors=gate_color, linewidth=1.5, zorder=0)

    for x_obs in [20, 40]:
        circle = patches.Circle((x_obs, 0), 4, color=color_obs, alpha=0.5,
                                label='Obstacle' if x_obs == 20 else "", zorder=5)
    ...
    for i in range(90):
        ax2.plot(ensemble_nc[i, 10, :], -ensemble_nc[i, 12, :], color=color_nom, alpha=0.15, lw=0.5, zorder=3)
        ax2.plot(ensemble_c[i, 10, :], -ensemble_c[i, 12, :], color=color_rob, alpha=0.15, lw=0.5, zorder=3)

    ax2.set_xlabel('X (m)', fontsize=8)
    ax2.set_ylabel('Z (m)', fontsize=8)
    ax2.set_xlim([0, 60])
    ax2.set_ylim([-22.5, 2.5])
    ax2.grid(True, linestyle=':', alpha=0.5)
    ax2.set_aspect('equal')
    plt.savefig("robustness_obstacles_states.svg", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    legend_elements = [
        # 1. The Design Means (Show as solid even if plotted dashed)
        Line2D([0], [0], color=color_nom, linestyle='-', linewidth=2, label='Nominal Design'),
        Line2D([0], [0], color=color_rob, linestyle='-', linewidth=2, label='Robust Design'),
        Line2D([0], [0], marker='x', color='black', linestyle='none',
                                      markersize=4, label='Centre of Gravity'),
        Line2D([0], [0],
               marker='o',
               color='black',  # This handles the edge color
               markerfacecolor='none',  # This makes the inside hollow
               linestyle='none',
               markersize=4,  # Adjusted for visibility
               label='Neutral Point')
    ]
    ...
    fig_leg.savefig('common_legend.pdf', dpi=300, bbox_inches='tight')
    ...
    save_single_drone(color_nom, 1.000, 0.116, 0.054, "drone_nominal_obstacles.svg")
    save_single_drone(color_rob, 0.944, 0.180, 0.311, "drone_robust_obstacles.svg")
...
path_n = "../results/robustness_analysis/evaluation_nominal_obstacles.npz"
path_r = "../results/robustness_analysis/evaluation_robust_obstacles.npz"
