...
def plot_ensemble_top_view(ref_nocontrol, ref_control, ensemble_nc, ensemble_c):
    ...
    fig = plt.figure(figsize=(3.25, 3.75))
    fig.suptitle("Vertical Reversal", fontsize=10, fontweight='bold', y=1.00)
    gs = gridspec.GridSpec(1, 2)


    color_rob = '#0072B2'  # Blue for Robust
    color_nom = '#D55E00'  # Orange for Nominal
    gate_color = '#009E73'  # Green
    ...
    ax1.plot(ref_nocontrol[IDX_Y, :], -ref_nocontrol[IDX_Z, :], color=color_nom,
             linestyle='--', linewidth=2, zorder=4)
    ax1.plot(ref_control[IDX_Y, :], -ref_control[IDX_Z, :], color=color_rob,
             linestyle='--', linewidth=2, zorder=4)

    for i in range(90):
        ax1.plot(ensemble_nc[i, IDX_Y, :], -ensemble_nc[i, IDX_Z, :],
                 color=color_nom, alpha=0.15, linewidth=0.5, zorder=3)

    for i in range(90):
        ax1.plot(ensemble_c[i, IDX_Y, :], -ensemble_c[i, IDX_Z, :],
                 color=color_rob, alpha=0.15, linewidth=0.5, zorder=3)

    gate_y, gate_z = ref_control[IDX_Y, -1], -ref_control[IDX_Z, -1]
    gate_radius = 1.0

    gate_circle = patches.Circle((gate_y, gate_z), gate_radius,
                                 edgecolor=gate_color, facecolor='none',
                                 linewidth=1.5, linestyle='-', zorder=5)
    ax1.add_patch(gate_circle)

    ax1.set_xlabel('Y (m)', fontsize=8)
    ...
    ax2.hlines([gate_y - gate_radius, gate_y + gate_radius], gate_x - 1, gate_x + 1,
    ...
    plt.tight_layout()
    plt.savefig("robustness_vert_states_top.svg", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    save_single_drone(color_nom, 1.000, 0.071, 0.060, "drone_nominal_vert.svg")
    save_single_drone(color_rob, 0.996, 0.155, 0.328, "drone_robust_vert.svg")
...
path_n = "../results/robustness_analysis/evaluation_nominal_verticalreversal.npz"
path_r = "../results/robustness_analysis/evaluation_robust_verticalreversal.npz"
