...
def objective_function(payload):
    ...
    try:
        ...
        x_guess = scale * np.sin(np.linspace(0, np.pi, N))
        y_guess = np.zeros(N)
        z_guess = scale * np.cos(np.linspace(0, np.pi, N))
        ...
        print(
            f"Move to final position completed in {time_final_opt:.4f} s, energy: {energy_final_opt:.4f} and mean cost: {total_cost:.4f}, with a semispan of: {semispan:.4f}, a chord of: {chord:.4f}, and a wing position of: {x_wing:.4f}")
...
if __name__ == '__main__':
    ...
    results_dir = "results/results_nominal_codesign_verticalreversal"
